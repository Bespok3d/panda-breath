# panda-breath-plugin: instructions for AI assistants
...
This repo publishes a single plugin, panda-breath: a standalone Bespok3d plugin that drives a BIQU
Panda Breath chamber heater as a standard Klipper heater on any Klipper printer (not U1-specific). Solo
repo, it publishes a single atom into `Bespok3d/main-index`. The plugin carries Python: it vendors the
Klipper module `panda_breath.py` from justinh-rahb/pandabreath-klipper (GPL-3.0, pinned commit).
